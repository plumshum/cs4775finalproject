...
mutMatrixGlobal = [[-1.0, 1.0 / 3, 1.0 / 3, 1.0 / 3], [1.0 / 3, -1.0, 1.0 / 3, 1.0 / 3],
                   [1.0 / 3, 1.0 / 3, -1.0, 1.0 / 3], [1.0 / 3, 1.0 / 3, 1.0 / 3, -1.0]]
...
def handler_helper(ctx):
    if ctx["returnLK"]:
        ctx["cumulPartLk"] += (ctx["bLen1"] + ctx["bLen2"]) * (
                    ctx["cumulativeRateUsed"][ctx["pos"]] - ctx["cumulativeRateUsed"][ctx["newPos"]])
        if ctx["usingErrorRate"]:
            if ctx["fromTip1"] or ctx["fromTip2"]:
                if ctx["errorRateSiteSpecific"]:
                    ctx["cumErrorRate"] = ctx["cumulativeErrorRateUsed"][ctx["newPos"]] - \
                                          ctx["cumulativeErrorRateUsed"][ctx["pos"]]
                else:
    ...
    if ctx["isUpDown"]:
        if ctx["usingErrorRate"]:
            if len(ctx["entry2"]) == 2:
                ...
                ctx["probVect"].append(
                    (ctx["entry2"][0], newEl, ctx["entry2"][2] + ctx["bLen2"], 0.0, ctx["entry2"][3]))
        else:
            if len(ctx["entry2"]) > 2:
...
def handler_other(ctx):
    ...
    ctx["flag1"] = (ctx["usingErrorRate"] and (ctx["entry1"][0] != 6) and (
                (len(ctx["entry1"]) > 2 and ctx["entry1"][-1]) or ctx["fromTip1"]))
    ctx["flag2"] = (ctx["usingErrorRate"] and (ctx["entry2"][0] != 6) and (
                (len(ctx["entry2"]) > 2 and ctx["entry2"][-1]) or ctx["fromTip2"]))
    ...
    if ctx["returnLK"]:
        if ctx["entry1"][0] == 4 and ctx["entry2"][0] == 4:
            if ctx["totLen2"] > ctx["bLen2"] or ctx["totLen1"] > ctx["bLen1"]:
                ctx["cumulPartLk"] += (ctx["totLen2"] - ctx["bLen2"] + ctx["totLen1"] - ctx["bLen1"]) * (
                            ctx["cumulativeRateUsed"][ctx["newPos"]] - ctx["cumulativeRateUsed"][ctx["pos"]])
                if ctx["usingErrorRate"]:
                    if ((not ctx["fromTip1"]) and ctx["flag1"]) or ((not ctx["fromTip2"]) and ctx["flag2"]):
                        if ctx["errorRateSiteSpecific"]:
                            ctx["cumErrorRate"] = ctx["cumulativeErrorRateUsed"][ctx["pos"]] - \
                                                  ctx["cumulativeErrorRateUsed"][ctx["newPos"]]
                        else:
            ...
            if ctx["useRateVariation"]:
                ctx["cumulPartLk"] -= ctx["mutMatricesUsed"][ctx["pos"]][ctx["refNucToPass"]][ctx["refNucToPass"]] * (
                            ctx["bLen2"] + ctx["bLen1"])
            else:
                ctx["cumulPartLk"] -= ctx["mutMatrix"][ctx["refNucToPass"]][ctx["refNucToPass"]] * (
                            ctx["bLen2"] + ctx["bLen1"])
            if ctx["usingErrorRate"] and ((ctx["entry1"][0] != ctx["entry2"][0]) or ctx["entry1"][0] == 6) and (
                    ctx["fromTip1"] or ctx["fromTip2"]):
                if ctx["errorRateSiteSpecific"]:
    ...
    if ctx["entry2"][0] == ctx["entry1"][0] and ctx["entry2"][0] < 5:
        if ctx["entry1"][0] == 4:
            ...
            if ctx["returnLK"]:
                if ctx["useRateVariation"]:
                    ctx["cumulPartLk"] += ctx["mutMatricesUsed"][ctx["pos"]][ctx["entry1"][0]][ctx["entry1"][0]] * (
                                ctx["totLen1"] + ctx["totLen2"])
                else:
                    ctx["cumulPartLk"] += ctx["mutMatrix"][ctx["entry1"][0]][ctx["entry1"][0]] * (
                                ctx["totLen1"] + ctx["totLen2"])
                if ctx["usingErrorRate"]:
                    if ((not ctx["fromTip1"]) and ctx["flag1"]) or ((not ctx["fromTip2"]) and ctx["flag2"]):
                        if ctx["errorRateSiteSpecific"]:
    ...
    elif (not ctx["totLen1"]) and (not ctx["totLen2"]) and ctx["entry1"][0] < 5 and ctx["entry2"][0] < 5 and (
    not ctx["flag1"]) and (not ctx["flag2"]):
        if ctx["returnLK"]:
        ...
        if ctx["i1"] <= 4:
            if ctx["totLen1"] or ctx["flag1"]:
                if ctx["isUpDown"] and len(ctx["entry1"]) > 3 + ctx["usingErrorRate"]:
                    ctx["newVec"] = getPartialVec(ctx["i1"], ctx["entry1"][2], ctx["mutMatrix"], ctx["errorRate"],
                                                  flag=ctx["flag1"])
                    for ctx["i"] in range(4):
                        ctx["newVec"][ctx["i"]] *= ctx["rootFreqs"][ctx["i"]]
                    if ctx["entry1"][3] + ctx["bLen1"]:
                        ctx["newVec"] = getPartialVec(6, ctx["entry1"][3] + ctx["bLen1"], ctx["mutMatrix"], 0,
                                                      vect=ctx["newVec"], upNode=True)
                else:
                    ctx["newVec"] = getPartialVec(ctx["i1"], ctx["totLen1"], ctx["mutMatrix"], ctx["errorRate"],
                                                  flag=ctx["flag1"], upNode=ctx["isUpDown"])
            else:
                ctx["newVec"] = [0.0, 0.0, 0.0, 0.0]
                ctx["newVec"][ctx["i1"]] = 1.0
        else:
            if ctx["totLen1"]:
                ctx["newVec"] = getPartialVec(6, ctx["totLen1"], ctx["mutMatrix"], 0, vect=ctx["entry1"][-1],
                                              upNode=ctx["isUpDown"])
            else:
        ...
        if ctx["i2"] == 6:
            ...
            if ctx["totLen2"] or ctx["flag2"]:
                ctx["newVec2"] = getPartialVec(ctx["i2"], ctx["totLen2"], ctx["mutMatrix"], ctx["errorRate"],
                                               flag=ctx["flag2"])
            else:
...
def mergeVectors(probVect1, bLen1, fromTip1, probVect2, bLen2, fromTip2,
    ...
    if returnLK:
        ...
        if usingErrorRate and errorRateSiteSpecific:
            ctx["cumulativeErrorRateUsed"] = ctx["cumulativeErrorRateGlobal"] if ctx[
                                                                                     "cumulativeErrorRateGlobal"] is not None else cumulativeErrorRateGlobal
    ...
    while True:
        ...
        if entry2[0] == 5:
            ...
            if entry1[0] == 4 and entry2[0] == 4:  # case entry1 and entry2 are R
                ...
                flag1 = (usingErrorRate and (entry1[0] != 6) and len(entry1) > 2 and entry1[
                    -1])  # flag1 true if error rate applies to entry1
                ...
                # entry1 is of type "O"
                    ...
                    if entry2[0] == 6:
                        coeff0 = entry1[-1][0] * entry2[-1][0] + entry1[-1][1] * entry2[-1][1] + entry1[-1][2] * \
                                 entry2[-1][2] + entry1[-1][3] * entry2[-1][3]
                        coeff1 = 0.0
                        for i in range(4):
                            for j in range(4):
...
def _recompute_down_at_node(tree, node_idx):
    ...
    if len(children) != 2:
        raise ValueError(
            f"updatePartials currently supports binary trees; node {node_idx} has {len(children)} children")
...
def stringForNode(tree, nextNode, nameNode, distB, binary, nameInternalNode, count0BLenNodesOnce, namesInTree,
                  performLineageAssignmentByRefPlacement):
...
def placeSampleOnTree(tree, newSampleName, newSampleDiffs, parentNode, childNode, lenParentToInternal,
                      lenInternalToChild, newLeafLen, ref_seq):
...
def main():
    ...
    for i in range(lref):
        refNuc = ref[i].upper()
        refIndeces.append(allelesDict[refNuc]) if refNuc in allelesDict else refIndeces.append(
            0)  # default to A if unknown
