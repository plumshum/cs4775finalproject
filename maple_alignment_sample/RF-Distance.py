...
def reRootTree(root,sample):
	#first find the node associated to the sample
	nodesToVisit=[root]
	sampleNode=None
	while nodesToVisit:
		node=nodesToVisit.pop()
	...
	if sampleNode==None:
		...
		return root
	#now create new root
	if sampleNode.up!=None:
		if sampleNode.up.up==None:
			if sampleNode==sampleNode.up.children[0]:
				sibling=sampleNode.up.children[1]
			else:
				sibling=sampleNode.up.children[0]
			sibling.dist+=sampleNode.dist
			sampleNode.dist=False
			print("Finished rerooting, nearby")
			return sampleNode.up
		newRoot=Tree()
		newRoot.name=""
		newRoot.dirty=True
		newRoot.add_child(sampleNode)
		newRoot.add_child(sampleNode.up)
		newRoot.dist=0.00000001
		newRoot.up=None
		oldDist=sampleNode.dist
		oldDistUp=sampleNode.up.dist
		oldUp=sampleNode.up
		oldUpUp=sampleNode.up.up
		sampleNode.dist=0.0
		sampleNode.up=newRoot
		oldUp.up=newRoot
		oldUp.dist=oldDist
		...
		while currentNode.up!=None:
			if currentChildChild==currentChild.children[0]:
				numChildChild=0
			else:
				numChildChild=1
			currentChild.children[numChildChild]=currentNode
			oldBLen=currentNode.dist
			oldPNode=currentNode.up
			currentNode.dist=currentBLen
		...
		if currentChildChild==currentChild.children[0]:
			...
			numChildChild=1
		if currentChild==currentNode.children[0]:
			numChild=0
		else:
			numChild=1
		currentChild.children[numChildChild]=currentNode.children[1-numChild]
		currentNode.children[1-numChild].up=currentChild
		currentNode.children[1-numChild].dist+=currentBLen
		print("Finished rerooting")
		return newRoot
...
def createNewick(node,binary=True,namesInTree=None):
	nextNode=node
	stringList=[]
	direction=0
	numLeaves=0
	while nextNode!=None:
		if nextNode.children:
			if direction==0:
				if nextNode.dist or binary or nextNode.up==None:
					stringList.append("(")
				...
				if nextNode.dist or binary or nextNode.up==None:
					if namesInTree==None:
						...
						stringList.append(")"+namesInTree[nextNode.name])
				...
				if binary:
					...
					if supportForIdenticalSequences or performLineageAssignment:
						if namesInTree==None:
							...
							stringList.append(stringForNode(nextNode,namesInTree[nextNode.name],0.0))
					else:
						if namesInTree==None:
							stringList.append(nextNode.name)
						else:
					...
					for s2 in nextNode.minorSequences[:-1]:
						...
						if supportForIdenticalSequences or performLineageAssignment:
							if namesInTree==None:
								...
								stringList.append(stringForNode(nextNode,namesInTree[s2],0.0))
						else:
							if namesInTree==None:
					...
					if supportForIdenticalSequences or performLineageAssignment:
						if namesInTree==None:
							...
							stringList.append(stringForNode(nextNode,namesInTree[nextNode.minorSequences[-1]],0.0))
					else:
						if namesInTree==None:
							stringList.append(nextNode.minorSequences[-1])
						else:
							stringList.append(namesInTree[nextNode.minorSequences[-1]])
					if namesInTree==None:
						stringList.append(":0.0)"+nextNode.name+"_MinorSeqsClade")
					else:
						stringList.append(":0.0)"+namesInTree[nextNode.name]+"_MinorSeqsClade")
				else:
					if nextNode.dist or nextNode.up==None:
						stringList.append("(")
					if supportForIdenticalSequences or performLineageAssignment:
						if namesInTree==None:
							stringList.append(stringForNode(nextNode,nextNode.name,0.0))
					...
					if nextNode.dist or nextNode.up==None:
						if namesInTree==None:
							...
							stringList.append(")"+namesInTree[nextNode.name]+"_MinorSeqsClade")
			else:
				if namesInTree==None:
					stringList.append(nextNode.name)
				else:
...
if performLineageAssignment:
		...
		if node.children:
			...
			if direction==0:
				...
				if lineage=="" or len(mostAncestralLineages)==0 or len(allLineages)==0:
					print("empty lineage assignment")
					print(node.name)
					print(lineage)
	...
	file.close()
	newickString=createNewick(tree1,binary=binaryTree,namesInTree=None)
...
def cutAndPasteNode(node,bestNode,bestBranchLengths,bestLK):
	...
	if (writeTreesToFileEveryTheseSteps>0) and (topologyChanges[0]%writeTreesToFileEveryTheseSteps)==0:
		...
		intermediateTreesFile.write(createNewick(currentRoot,binary=binaryTree,namesInTree=None)+"\n")
							...
							if not totLen2:
								...
								if entry2[0]==6:
									...
									if nodeIsLeaf and usingErrorRate:
										...
										if trackMutations:
											if (errorProb>minMutProb):
												node.errors.append((4,pos+1,i2,errorProb))
										if errorRateSiteSpecific:
											errorCountSites[pos]+=errorProb
...
newickString=createNewick(t1,binary=binaryTree,namesInTree=None)
